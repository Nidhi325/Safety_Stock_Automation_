...
# ── Input paths ───────────────────────────────────────────────────────
HISTORICAL_FILE = str(PROJECT_ROOT / 'Data' / 'Final_dataset.csv')
MONTHLY_FILE = str(PROJECT_ROOT / 'final_month_data' / 'final_month_dataprocessed_monthly_data.csv')

# ── Output path ───────────────────────────────────────────────────────
OUTPUT_FILE = str(PROJECT_ROOT / 'Data_SES' / 'updated_historical_dataset.csv')
...
# ── Recalculate CV and XYZ_Class over the entire history ──────────────
log_step('Recalculating ABC and XYZ classifications over entire updated history...')

# Calculate CV
avg_demand = combined_df.groupby('material_id')['Demand'].transform('mean')
std_demand = combined_df.groupby('material_id')['Demand'].transform('std').fillna(0)
combined_df['CV'] = (std_demand / (avg_demand + 1e-6)).round(4)

# Apply XYZ classification
def xyz_class_func(cv):
    if cv <= 0.5: return 'X'
    elif cv <= 1.0: return 'Y'
    else: return 'Z'

combined_df['XYZ_Class'] = combined_df['CV'].apply(xyz_class_func)

# Recalculate ABC_Class over the entire history
abc_df = (
    combined_df[['material_id', 'Demand', 'Moving_price']]
    .groupby('material_id', as_index=False)
    .agg(avg_demand=('Demand', 'mean'), moving_price=('Moving_price', 'first'))
)
# Ensure moving price is numeric
abc_df['moving_price'] = pd.to_numeric(abc_df['moving_price'], errors='coerce').fillna(0)
...
abc_df['Contribution_Pct']  = (abc_df['Consumption_Value'] / (abc_df['Consumption_Value'].sum() + 1e-6)) * 100
abc_df['Cumulative_Pct']    = abc_df['Contribution_Pct'].cumsum()

def abc_class_func(cumulative_pct):
    if cumulative_pct <= 80: return 'A'
    elif cumulative_pct <= 95: return 'B'
    else: return 'C'

abc_df['ABC_Class'] = abc_df['Cumulative_Pct'].apply(abc_class_func)

# Merge ABC_Class back
if 'ABC_Class' in combined_df.columns:
    combined_df.drop(columns=['ABC_Class'], inplace=True)
